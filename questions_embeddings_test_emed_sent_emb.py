...
class EmbeddingsGenerator:
    ...
    def initialize_chromadb(self):
        ...
        persist_directory_questions = os.path.join(
            current_dir, 'chromadb_store_sentence_embedding_emed_questions'
        )
        persist_directory_entities = os.path.join(
            current_dir, 'chromadb_store_wikidata_entities_sentence_embedding_emed'
        )
        persist_directory_relationships = os.path.join(
            current_dir, 'chromadb_store_wikidata_relationships_sentence_embedding_emed'
        )
        ...
        try:
            self.client_questions.delete_collection(name="sentence_embedding_emed_questions_collection")
            print("Deleted existing 'sentence_embedding_emed_questions_collection' to avoid dimension mismatches.")
        except Exception:
            pass

        try:
            self.client_entities.delete_collection(name="wikidata_entities_sentence_embedding_emed")
            print("Deleted existing 'wikidata_entities_sentence_embedding_emed' collection to avoid dimension mismatches.")
        except Exception:
            pass

        try:
            self.client_relationships.delete_collection(name="wikidata_relationships_sentence_embedding_emed")
            print("Deleted existing 'wikidata_relationships_sentence_embedding_emed' collection to avoid dimension mismatches.")
        except Exception:
            pass

        # Create new collections
        self.collection_questions = self.client_questions.create_collection(
            name="sentence_embedding_emed_questions_collection"
        )
        print("Created new 'sentence_embedding_emed_questions_collection'.")

        self.collection_entities = self.client_entities.create_collection(
            name="wikidata_entities_sentence_embedding_emed"
        )
        print("Created new 'wikidata_entities_sentence_embedding_emed' collection.")

        self.collection_relationships = self.client_relationships.create_collection(
            name="wikidata_relationships_sentence_embedding_emed"
        )
        print("Created new 'wikidata_relationships_sentence_embedding_emed' collection.")
    ...
    def store_question_embeddings(self, items):
        ...
        pbar = tqdm(total=total_items, desc="Storing question embeddings")
        ...
        for start_idx in range(0, total_items, batch_size):
            ...
            # Time per question in batch
            batch_time = time.time() - batch_start_time
            total_embedding_time += batch_time
            avg_time_per_question = batch_time / len(batch_items)
        ...
        avg_time_per_question_total = total_embedding_time / total_items
        print(f"Average time per question embedding: {avg_time_per_question_total:.4f} seconds")
        print(f"Finished storing question embeddings in ChromaDB. Time taken: {total_time:.2f} seconds.")
    ...
    def store_wikidata_embeddings(self, items, is_entity=True):
        ...
        total_items = len(items)
        entity_type = 'entity' if is_entity else 'property'
        pbar = tqdm(total=total_items, desc=f"Storing {entity_type} embeddings")
        ...
        end_time = time.time()
        total_time = end_time - start_time
        avg_time_per_item_total = total_embedding_time / total_items
        print(f"Average time per {entity_type} embedding: {avg_time_per_item_total:.4f} seconds")
        print(f"Finished storing embeddings in ChromaDB. Time taken: {total_time:.2f} seconds.")
        print(f"Total {entity_type}s stored: {collection.count()}")
...
def main():
    ...
    try:
        ...
        # Path to your test_emed_q.xlsx file
        excel_file_path = '/app/datafinal/test_emed_q.xlsx'
        ...
        print(f"Question embeddings stored successfully in ChromaDB.\n")
        ...
        print(f"Wikidata entity embeddings stored successfully.\n")
        ...
        print(f"Wikidata property embeddings stored successfully.\n")
        ...
        for idx, question in enumerate(tqdm(questions, desc="Processing questions")):
            ...
            if result_data and 'embeddings' in result_data and len(result_data['embeddings']) > 0:
                ...
                # Query the entities and relationships
                n_results = 10  

                question_similarity_start = time.time()
                try:
                ...
                question_similarity_time = time.time() - question_similarity_start
                total_similarity_time += question_similarity_time
        ...
        end_time = time.time()
        total_time = end_time - start_time
        avg_similarity_time_per_question = total_similarity_time / len(questions)
        print(f"Average time per question similarity computation: {avg_similarity_time_per_question:.4f} seconds")
        print(f"Finished querying embeddings and writing results. Time taken: {total_time:.2f} seconds.\n")

        # Write results to JSON file
        with open('result_sentence_embedding_emed.json', 'w', encoding='utf-8') as json_file:

...
QUESTION_BATCH_SIZE = 1024   # for processing questions in batches
ENTITY_BATCH_SIZE = 8192    # increased entity/relation batch size
...
class SimilarityModel(nn.Module):
    ...
    def forward(self, question_embedding, batch_embeddings):
        ...
        # We assume both are normalized. Compute dot product:
        # Output: (1, B)
...
class SimilarityFinder:
    def __init__(self):
        ...
        if torch.cuda.is_available() and available_gpus > 1:
            logger.info(f"Using {available_gpus} GPUs for computation via DataParallel.")
    ...
    def load_embeddings(self):
        """Load embeddings in memory-mapped mode and keep them on CPU"""
        try:
            log_memory_usage()
    ...
    def load_questions_and_embeddings(self):
        """Load questions and their embeddings from ChromaDB's parquet files"""
        try:
            ...
            # Convert to torch tensor and normalize on CPU
    ...
    def process_batch(self, embeddings, metadata, question_embedding_gpu, start_idx, batch_size):
        """Process a batch of embeddings for similarity search using the model (DataParallel)"""
        try:
    ...
    def process_question(self, q_idx):
        ...
        start_time = time.time()  # <-- START timing for one question
        try:
            ...
            end_time = time.time()  # <-- END timing for one question
            logger.info(f"Time taken to process question index {q_idx} (ID: {question_id}): "
            ...
            end_time = time.time()
            logger.info(f"Time taken (with error) for question index {q_idx} (ID: {question_id}): "
                        f"{end_time - start_time:.4f} seconds")
            return question_id, {
    ...
    def find_similar_items(self):
        ...
        # List to collect processing times for each question
        question_times = []

        # Start timing for all questions
        start_time_all = time.time()
        
        try:
            # Process questions in batches of QUESTION_BATCH_SIZE
            for q_start_idx in tqdm(range(0, total_questions, QUESTION_BATCH_SIZE), desc="Processing questions"):
                ...
                for q_idx in range(q_start_idx, q_end_idx):
                    ...
                    # Accumulate the time for this question
...
def main():
    try:
        # Force usage of GPUs 0-4 (adjust if you have a different number of GPUs)
        os.environ["CUDA_VISIBLE_DEVICES"] = "0,1,2,3,4"

        # Debug prints for CUDA availability
        logger.info("Checking CUDA availability in main...")
        logger.info(f"torch.cuda.is_available(): {torch.cuda.is_available()}")
        logger.info(f"torch.cuda.device_count(): {torch.cuda.device_count()}")

        # Start with memory usage logging

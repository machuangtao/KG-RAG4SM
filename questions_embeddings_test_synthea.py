...
class EmbeddingsGenerator:
    def __init__(self):
        ...
        self.embedding_shape_printed = False  # Flag to print embedding shape once
    ...
    def initialize_chromadb(self):
        ...
        persist_directory_questions = os.path.join(
            current_dir, 'chromadb_store_test_synthea_questions'
        )
        persist_directory_entities = os.path.join(
            current_dir, 'chromadb_store_wikidata_entities_test_synthea'
        )
        persist_directory_relationships = os.path.join(
            current_dir, 'chromadb_store_wikidata_relationships_test_synthea'
        )
        ...
        try:
            self.client_questions.delete_collection(name="test_synthea_questions_collection")
            print("Deleted existing 'test_synthea_questions_collection' to avoid dimension mismatches.")
        except Exception:
            pass

        try:
            self.client_entities.delete_collection(name="wikidata_entities_test_synthea")
            print("Deleted existing 'wikidata_entities_test_synthea' collection to avoid dimension mismatches.")
        except Exception:
            pass

        try:
            self.client_relationships.delete_collection(name="wikidata_relationships_test_synthea")
            print("Deleted existing 'wikidata_relationships_test_synthea' collection to avoid dimension mismatches.")
        except Exception:
            pass

        # Create new collections
        self.collection_questions = self.client_questions.create_collection(
            name="test_synthea_questions_collection"
        )
        print("Created new 'test_synthea_questions_collection'.")

        self.collection_entities = self.client_entities.create_collection(
            name="wikidata_entities_test_synthea"
        )
        print("Created new 'wikidata_entities_test_synthea' collection.")

        self.collection_relationships = self.client_relationships.create_collection(
            name="wikidata_relationships_test_synthea"
        )
        print("Created new 'wikidata_relationships_test_synthea' collection.")
...
def main():
    ...
    try:
        ...
        # Path to your test_synthea_q.xlsx file
        excel_file_path = '/app/datafinal/test_synthea_q.xlsx'
        ...
        with open('result_test_synthea.json', 'w', encoding='utf-8') as json_file:

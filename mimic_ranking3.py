...
def main():
    ...
    #mimic_dir = os.path.join(collections_base, "mimic_ques_embedding_full2")
    mimic_dir = os.path.join(base_dir, "mimic_ques_embedding_full2")
    
    # Load mimic embeddings
    print("Loading mimic embeddings...")
    mimic_embeddings_path = os.path.join(mimic_dir, "chroma-embeddings.parquet")
    mimic_embeddings = load_embeddings_from_parquet(mimic_embeddings_path)
    print(f"Loaded mimic embeddings: {mimic_embeddings.shape[0]} samples.")
    ...
    # Assign questions to GPUs (2 questions per GPU for 10 questions)
    max_questions = mimic_embeddings.shape[0]  # Change this to mimic_embeddings.shape[0] to process all questions
    max_questions = min(max_questions, mimic_embeddings.shape[0])
    ...
    for chunk_start in tqdm(range(0, num_triplet2, triplet2_chunk_size), desc="Processing triplet2 chunks", unit="chunk"):
        ...
        # Normalize questions
        questions = mimic_embeddings[:max_questions]
    ...
    for gpu_id, (gpu_idx, start_q, end_q) in enumerate(gpu_question_assignments):
        ...
        for q in range(start_q, end_q):
            ...
            q_embedding = mimic_embeddings[q]
    ...
    output_json_path = os.path.join(base_dir, "mimic_top10_similar2.json")

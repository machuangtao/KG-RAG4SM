...
class SimilarityFinder:
    ...
    def process_question(self, q_idx):
        ...
        # Timing changes: Record start time for this question
        ...
        # Timing changes: Record end time and compute elapsed time
        ...
        return question_id, {
            ...
            "processing_time": question_time  # Store individual question time
        }
    ...
    def find_similar_items(self):
        ...
        # Timing changes: List to store per-question times
        question_times = []
        total_start_time = time.time()

        # Outer loop processes in batches for large number of questions
        for q_start_idx in tqdm(range(0, total_questions, QUESTION_BATCH_SIZE), desc="Processing question batches"):
            ...
            # Inner loop to process individual questions with a tqdm progress bar
            for q_idx in tqdm(range(q_start_idx, q_end_idx), desc="Processing individual questions", leave=False):
                try:
                    ...
                    # Store the individual question time
                ...
                # You can still log intermediate progress here if needed
                # if q_idx % 10 == 0:
        ...
        # Print timing results after all questions are processed
        logger.info("======== Timing Information ========")
        logger.info(f"Total processing time for all questions: {total_time:.2f} seconds")
        logger.info(f"Average processing time per question: {avg_time_per_question:.4f} seconds")

        # Also log the average of the per-question times recorded at the question level
        if question_times:
    ...
    def save_results(self, results):
        ...
        try:
            json_path = "mimic_wikidata_similar_full2.json"
            if TEST_MODE:
                json_path = "mimic_wikidata_similar_test2.json"
            
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(results, f, ensure_ascii=False, indent=2)
            logger.info(f"Results saved to {json_path}")
            
            txt_path = "mimic_wikidata_similar_full2.txt"
            if TEST_MODE:
                txt_path = "mimic_wikidata_similar_test2.txt"
            ...
            with open(txt_path, 'w', encoding='utf-8') as f:
                for question_id, data in results.items():
                    ...
                    if 'error' in data:
                        ...
                        # If timing info stored per question is desired in the text file:

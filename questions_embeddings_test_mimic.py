...
class EmbeddingsGenerator:
    ...
    def initialize_chromadb(self):
        ...
        persist_directory_questions = os.path.join(
            current_dir, 'chromadb_store_test_emed_questions'
        )
        persist_directory_entities = os.path.join(
            current_dir, 'chromadb_store_wikidata_entities_test_emed'
        )
        persist_directory_relationships = os.path.join(
            current_dir, 'chromadb_store_wikidata_relationships_test_emed'
        )
        ...
        try:
            self.client_questions.delete_collection(name="test_emed_questions_collection")
            print("Deleted existing 'test_emed_questions_collection' to avoid dimension mismatches.")
        except Exception:
            pass

        try:
            self.client_entities.delete_collection(name="wikidata_entities_test_emed")
            print("Deleted existing 'wikidata_entities_test_emed' collection to avoid dimension mismatches.")
        except Exception:
            pass

        try:
            self.client_relationships.delete_collection(name="wikidata_relationships_test_emed")
            print("Deleted existing 'wikidata_relationships_test_emed' collection to avoid dimension mismatches.")
        except Exception:
            pass

        # Create new collections
        self.collection_questions = self.client_questions.create_collection(
            name="test_emed_questions_collection"
        )
        print("Created new 'test_emed_questions_collection'.")

        self.collection_entities = self.client_entities.create_collection(
            name="wikidata_entities_test_emed"
        )
        print("Created new 'wikidata_entities_test_emed' collection.")

        self.collection_relationships = self.client_relationships.create_collection(
            name="wikidata_relationships_test_emed"
        )
        print("Created new 'wikidata_relationships_test_emed' collection.")
...
def main():
    ...
    try:
        ...
        with open('result_test_emed.json', 'w', encoding='utf-8') as json_file:

...
class EmbeddingsGenerator:
    ...
    def initialize_chromadb(self):
        ...
        print("Created new 'questions_collection'.")
        ...
        print("Created new 'wikidata_entities' collection.")
        ...
        print("Created new 'wikidata_relationships' collection.")
...
"""
        ...
        with open('result_manhattan.txt', 'w', encoding='utf-8') as result_file:

            # Retrieve all entity embeddings
            entity_results = embeddings_generator.collection_entities.get()
            entity_embeddings = entity_results['embeddings']
            entity_ids_list = entity_results['ids']

            # Retrieve all relationship embeddings
            relationship_results = embeddings_generator.collection_relationships.get()
            relationship_embeddings = relationship_results['embeddings']
            relationship_ids_list = relationship_results['ids']
            ...
            for idx, question in enumerate(tqdm(questions, desc="Processing questions")):
                ...
                if result_data and 'embeddings' in result_data and len(result_data['embeddings']) > 0:
                    ...
                    # Compute Manhattan distances to entities
                    distances = []
                    for emb in entity_embeddings:
                        distance = np.sum(np.abs(question_embedding - np.array(emb)))
                        distances.append(distance)
                    # Get top N results
                    n_results = 10
                    top_indices = np.argsort(distances)[:n_results]
                    top_entity_ids = [entity_ids_list[i] for i in top_indices]
                    top_entity_distances = [distances[i] for i in top_indices]

                    # Print and save similar entities
                    output_str = "Top similar Wikidata entities:\n"
                    print(output_str)
                    result_file.write(output_str)
                    for sim_id, distance in zip(top_entity_ids, top_entity_distances):
                        entity_id = sim_id
                    ...
                    # Compute Manhattan distances to relationships
                    distances = []
                    for emb in relationship_embeddings:
                        distance = np.sum(np.abs(question_embedding - np.array(emb)))
                        distances.append(distance)
                    # Get top N results
                    top_indices = np.argsort(distances)[:n_results]
                    top_relationship_ids = [relationship_ids_list[i] for i in top_indices]
                    top_relationship_distances = [distances[i] for i in top_indices]

                    # Print and save similar relationships
                    output_str = "Top similar Wikidata relationships:\n"
                    print(output_str)
                    result_file.write(output_str)
                    for sim_id, distance in zip(top_relationship_ids, top_relationship_distances):
                        prop_id = sim_id

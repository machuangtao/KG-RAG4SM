...
TEST_MODE = False   # Set to False as we want actual logic
NUM_TEST_QUESTIONS = 100  # Not used now since we process just one question
...
class SimilarityFinder:
    ...
    def load_questions_and_embeddings(self):
        """Load questions and their embeddings from a parquet file."""
        logger.info("Loading questions and embeddings...")
        log_memory_usage()
        try:
            chroma_dir = os.path.join(os.getcwd(), 'synthea_ques_embedding_full2')
    ...
    def process_one_question(self):
        """Process only the first question."""
        total_questions = len(self.questions['ids'])
        if total_questions == 0:
            logger.warning("No questions available.")
            return
        
        # Process only the first question and measure time
        ...
        end_time = time.time()
        elapsed_time_for_one = end_time - start_time
        estimated_time_for_all = elapsed_time_for_one * total_questions
        estimated_time_for_all_minutes = estimated_time_for_all / 60.0

        # Print the results for this one question
        logger.info("=== Results for the first question ===")
        logger.info(f"Question ID: {question_id}")
        logger.info(f"Question: {question_text}")
        logger.info("Top 10 Similar Entities:")
        for ent in similar_entities[:10]:
            logger.info(f"- {ent['id']}: {ent['text']} (Score: {ent['score']:.4f})")

        logger.info("Top 10 Similar Relations:")
        for rel in similar_relations[:10]:
            logger.info(f"- {rel['id']}: {rel['text']} (Score: {rel['score']:.4f})")

        # Print timing information, including minutes
        logger.info("=== Timing Information ===")
        logger.info(f"Time taken for processing the first question: {elapsed_time_for_one:.4f} seconds")
        logger.info(f"Estimated time for all {total_questions} questions: {estimated_time_for_all:.4f} seconds ({estimated_time_for_all_minutes:.2f} minutes)")
